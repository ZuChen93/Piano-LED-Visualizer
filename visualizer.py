...
# from mido import MidiFile, Message, tempo2bpm, MidiTrack, MetaMessage, Ports
RST = 27
DC = 25
BL = 24
bus = 0
device_pin = 0
W, H = (240, 240)
fonthour = ImageFont.truetype('/usr/share/fonts/truetype/freefont/FreeSansBold.ttf', 40)
font = ImageFont.truetype('/usr/share/fonts/truetype/freefont/FreeSansBold.ttf', 18)
font_cn = ImageFont.truetype("/usr/share/fonts/truetype/freefont/FreeSansBold.ttf", 20)
font_title = ImageFont.truetype("/usr/share/fonts/truetype/freefont/FreeMonoBold.ttf", 20)
...
class LedStrip:
    ...
    def set_adjacent_colors(self, note, color):
        if (ledsettings.adjacent_mode == "RGB" and color != 0):
            color = Color(int(ledsettings.adjacent_green), int(ledsettings.adjacent_red),
                          int(ledsettings.adjacent_blue))
...
class MenuLCD:
    def __init__(self, xml_file_name):
        ...
        self.LCD.ShowImage(self.image, 0, 0)
    ...
    def show(self, position="default", back_pointer_location=False):
        ...
        title = position.replace("_", " ")
        self.draw.text(((W - self.draw.textsize(title, font_title)[0] - font_title.getoffset(title)[0]) / 2, 3),
                       text=title,
                       fill="Crimson",
                       font=font_title)
        self.draw.line([(0, 26), (W, 26)], fill="Crimson", width=2)

        # getting list of items in current menu
        staffs = self.DOMTree.getElementsByTagName(position)
        text_margin_top = 30
        i = 0
        list_count = len(staffs) - 1

        if self.pointer_position > 7:
            self.menu_offset = self.pointer_position - 7
        else:
            self.menu_offset = -1
        ...
        for staff in staffs:
            ...
            if (self.pointer_position > 7 and i < self.menu_offset):
                self.draw.polygon([(220, 45), (215, 50), (225, 50)], fill=self.text_color)
            ...
            if (not back_pointer_location):
                if (i == self.pointer_position):
                    ...
                    self.draw.rectangle([(0, text_margin_top), (W, text_margin_top + 24)], fill="Crimson")
                    self.draw.text((3, text_margin_top - 2), ">", fill=self.text_color, font=font_cn)
            ...
            if (i == 7 or self.pointer_position < list_count and list_count > 7):
                self.draw.polygon([(220, 220), (215, 215), (225, 215)], fill=self.text_color)
            ...
            if (self.pointer_position == i and len(sid) > 24):
                tobecontinued = ".."
                if (refresh == 1):
                    try:
                        self.cut_count += 1
            ...
            i += 1
            self.draw.text((20, text_margin_top), sid[cut:(20 + cut)] + tobecontinued, fill=self.text_color,
                           font=font_cn)
            text_margin_top += 26

        # displaying color example
        if (self.currentlocation == "RGB"):
            self.draw.text((60, 180), str(ledsettings.get_colors()), fill=self.text_color, font=font_cn)
            self.draw.rectangle([(0, 200), (240, 240)], fill="rgb(" + str(ledsettings.get_colors()) + ")")

        if ("RGB_Color" in self.currentlocation):
            self.draw.text((60, 180), str(ledsettings.get_multicolors(self.currentlocation.replace('RGB_Color', ''))),
                           fill=self.text_color, font=font_cn)
            self.draw.rectangle([(0, 200), (240, 240)], fill="rgb(" + str(
                ledsettings.get_multicolors(self.currentlocation.replace('RGB_Color', ''))) + ")")

        if ("Backlight_Color" in self.currentlocation):
            self.draw.text((60, 180), str(ledsettings.get_backlight_colors()), fill=self.text_color, font=font_cn)
            self.draw.rectangle([(0, 200), (240, 240)], fill="rgb(" + str(ledsettings.get_backlight_colors()) + ")")

        if ("Custom_RGB" in self.currentlocation):
            self.draw.text((60, 180), str(ledsettings.get_adjacent_colors()), fill=self.text_color, font=font_cn)
            self.draw.rectangle([(0, 200), (240, 240)], fill="rgb(" + str(ledsettings.get_adjacent_colors()) + ")")

        if ("Multicolor" in self.currentlocation):
            try:
                self.draw.rectangle([(200, 100), (220, 160)], fill="rgb(" + str(
                    ledsettings.get_multicolors(self.current_choice.replace('Color', ''))) + ")")
            except:
                pass
        ...
        if ("Key_range" in self.currentlocation):
            if (self.current_choice == "Start"):
                try:
                    self.draw.text((10, 50), str(
                        ledsettings.multicolor_range[int(self.currentlocation.replace('Key_range', '')) - 1][0]),
                                   fill=self.text_color)
                except:
                    pass
            else:
                self.draw.text((10, 50), str(
                    ledsettings.multicolor_range[int(self.currentlocation.replace('Key_range', '')) - 1][1]),
                               fill=self.text_color)
        self.draw = ImageDraw.Draw(self.image)
    ...
    def render_screensaver(self, hour, date, cpu, cpu_average, ram, temp, cpu_history=[]):
        ...
        w, h = self.draw.textsize(hour, fonthour)
        print(w, h)
        x = (W - w - fonthour.getoffset(hour)[0]) / 2
        y = (H - h) / 4
        self.draw.text((x, y), hour, self.text_color, font=fonthour)
        # self.draw.text((4, 5), hour, fill=self.text_color, font=fonthour)
        self.draw.text(((W - self.draw.textsize(date, font)[0] - fonthour.getoffset(date)[0]) / 2, 30), date,
        ...
        for cpu_chart in cpu_history:
            ...
            self.draw.line([(c, 100 + previous_height), (c + 10, 100 + height)], fill="Red", width=1)
            previous_height = height
            c += 10

        self.draw.text((3, 160), "CPU: " + str(cpu) + "% (" + str(cpu_average) + "%)", fill=self.text_color, font=font)
        self.draw.text((3, 180), "RAM usage: " + str(ram) + "%", fill=self.text_color, font=font)
        self.draw.text((3, 200), "Temp: " + str(temp) + " ℃", fill=self.text_color, font=font)
        self.LCD.ShowImage(self.image, 0, 0)
    ...
    def change_value(self, value):
        ...
        if ("Key_range" in self.currentlocation):
            ledsettings.change_multicolor_range(self.current_choice, self.currentlocation,
...
class LedSettings:
    def __init__(self):
        ...
        self.backlight_brightness = float(usersettings.get_setting_value("backlight_brightness"))
        self.backlight_brightness_percent = float(usersettings.get_setting_value("backlight_brightness_percent"))
...
while True:
    ...
    for msg in midipending:
        ...
        if (ledsettings.color_mode == "Rainbow"):
            red = get_rainbow_colors(int(
                (int(((note - 20) * 2 - note_offset)) + ledsettings.rainbow_offset + int(timeshift)) * (
                        float(ledsettings.rainbow_scale) / 100)) & 255, "red")
            green = get_rainbow_colors(int(
                (int(((note - 20) * 2 - note_offset)) + ledsettings.rainbow_offset + int(timeshift)) * (
                        float(ledsettings.rainbow_scale) / 100)) & 255, "green")
            blue = get_rainbow_colors(int(
                (int(((note - 20) * 2 - note_offset)) + ledsettings.rainbow_offset + int(timeshift)) * (
                        float(ledsettings.rainbow_scale) / 100)) & 255, "blue")
        ...
        if (int(velocity) == 0 and int(note) > 0):
            ...
            if (ledsettings.mode == "Fading"):
                ...
                if (ledsettings.backlight_brightness > 0):
                    red = int(ledsettings.get_backlight_color("Red")) * (ledsettings.backlight_brightness_percent) / 100
                    green = int(ledsettings.get_backlight_color("Green")) * (
                        ledsettings.backlight_brightness_percent) / 100
                    blue = int(ledsettings.get_backlight_color("Blue")) * float(
                        ledsettings.backlight_brightness_percent) / 100
                    color = Color(int(green), int(red), int(blue))
            ...
            if (find_between(str(msg), "channel=", " ") == "12"):
                ...
                ledstrip.strip.setPixelColor(((note - 20) * 2 - note_offset), Color(int(int(green) / float(brightness)),
                                                                                    int(int(red) / float(brightness)),
                                                                                    int(int(blue) / float(brightness))))
                ledstrip.set_adjacent_colors(((note - 20) * 2 - note_offset), Color(int(int(green) / float(brightness)),
                                                                                    int(int(red) / float(brightness)),
                                                                                    int(int(blue) / float(brightness))))
            if (saving.isrecording == True):
